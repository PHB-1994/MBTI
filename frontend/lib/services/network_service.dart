...
class NetworkService {
  ...
  우리의 어플에는 문제가 없고, 데이터가 문제 있음을 전달

  2. 은행이나 보안 관련 어플은 인증 절차와 같이 사용 기간에는 wi-fi 사용 못하게 해야할 경우가 있음
  이럴 때 wifi 로 접속한 것이 아니라 모바일 데이터로만 접속을 해야지만 ok

  3. 어플 다운 / 대용량 다운 / 동영상 재생 유무 -> 스마트폰 기기나 어플에서
  wifi 가 연결되어야지만 다운이 되거나 동영상 자동 재생 처리 할 수 있게 설정
  ...
  //                                    final = 1회 나 일시적으로 호출할 때
  static final NetworkService _instance = NetworkService._internal();
  // 만약 외부에서 _instance 내부에 존재하는 데이터에 접근하려 하는 경우
  // 기존에 사용했던 방법처럼 NetworkService._instance NetworkService 에 존재하는 변수이름
  //                        과 같이 직접적으로 접근할 수 없음

  // service 라는 객체를 생성하여 NetworkService() 를 복제하여 담아서 사용
  // 자바에서 생성자를 이용하고, 게터세터를 이용해서 DB 에서 가져온 데이터를 사용하거나 저장하는 용도 비슷
  // ._internal() 프라이빗 생성자
  ...
  // 현재 연결 타입 가져오기
  // _connectivity =  network_service.dart 에서만 사용 가능한 변수명칭
  // api_service 나 main 과 같은 외부 .dart 파일은 접근 불가
...
Future<bool> hasInternet() async {
    try {
      final results = await InternetAddress.lookup('google.com'); // 보통은 본인 회사 홈페이지 작성하여 접속 확인
      // 우리 사이트가 없기 때문에 구글 봐지는지 테스트
