...
class ModelsApiService {
  ...
  static Future<List<Question>> getQuestions() async {
    ...
    .map((json) => Question.fromJson(json)) = 변환을 할 때 각 데이터 하나씩 json 이라는 변수이름에 담아서
  ...
  .map((en) {                     = 각 MapEntry 를 TestAnswer 로 변환
      return TestAnswer(
   ...
   만약에 TestAnswer 와 같은 응답 전용 객체를 java 에서 사용하지 않는다면 entries 작업까지 할 필요는 없음
...
class DynamicApiService {
  ...
  static Future<Map<String, dynamic>> submitTest(String userName, Map<int, String> answers) async {
    ...
    // 어플에서 선택한 질문 번호와 질문에 대한 답변을 [{질문1, 답변},{질문2, 답변}, {질문3, 답변}]
