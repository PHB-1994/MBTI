...
MbtiDimensions - MTBI 4가지 척도
  EI(외향-내향), SN(감각-직관), TF(사고-감정), JP(판단-인식)

ErrorMessage - 사용자에게 표시할 오류 메세지
  네트워크 오류, 서버 오류, 입력 오류 등 다양한 상황의 안내 메세지

  static const vs static final = 언제 값이결정되는가의 차이
  static const
    컴파일 시점에 값이 결정됨
    값이 코드에 하드코딩 되어 있어야 한다.
  ...
  static final
    ...
    Dart 은 컴파일러와 인터프리터 두 가지를 모두 지원하는 언어
...
class ApiConstants {
   ...
   *
   * 실제 기기용
   * http://192.168.x.x:8080/apu
   *
   * 백엔드 주소
   * https://도메인.도메인/api
   *
   * chrome(web) 이나 Edge(web) 으로 console.log 로 상황 확인하며 개발을 하고자 할 경우
   * flutter run -d chrome --web-port=개발자가 원하는 포트
   *
   * 프로젝트 루트에 .flutter 폴더 생성 후 chrome_device.json 파일 형태로
   * {
   *  "port": 51093 과 같은 포트번호 지정
   * }
   *
   * 환경변수 파일 사용(.env)를 이용하여 constants 상태 관리를 할 수 있다.
   */
