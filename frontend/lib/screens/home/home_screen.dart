...
class _HomeScreenState extends State<HomeScreen> {
  ...
  /*
  만약에 input 이나 Textarea 를 사용할 경우에는 사용자들이 작성한 값(value)를 읽고
  읽은 value 데이터를 가져오기 위해 기능 작성해야함
  -> dart 에서는 TextEditingController 객체를 미리 만들어놓았음

  사용방법
  1. 컨트롤러 상태를 담을 변수 공간 설정 _private 설정을 안해도 됨
  final TextEditingController _nameController = TextEditingController();

  2. TextField 에 연결
  TextField(
    controller: _nameController 와 같은 형태로 내부에서 작성된 value 를 연결하도록 설정
  )
  ...
  // state 상태 변화
  // initState() -> 위젯 화면을 보여줄 때 초기 상태 화면을 변화
  // ex 화면에서 backend 데이터를 가져오겠다. 로그인 상태를 복원하겠다.
  @override
  void initState() {
        ...
        return Scaffold(
              ...
              if (isloggedIn)
                PopupMenuButton<String>(
                  ...
                  itemBuilder: (context) => [
                    ...
                    PopupMenuItem(child: Text('내 기록보기'), value: 'history'),
                    PopupMenuDivider(),
          ...
          body: SingleChildScrollView(
            ...
            * */
                    SizedBox(
                      ...
                      child: TextField(
                        ...
                        onChanged: (value) {
                          ...
                          // 변수 값만 변경 -> 변수 값은 변화하지만 화면에 업데이트는 안됨
                    ...
                    11111을 입력하는 순간에도 계속 에러 메세지를 지워버리기 때문에
                    정삭적으로 _errorText 작동하나 마치 작동하지 않는 것처럼 보임
                    onChanged: (value) {
                      if(_errorText != null){
                        setState(() {
                          _errorText = null;
                    ...
                    /*
             div 와 성격이 같은 SizeBox 를 이용해서
             이전 결과 보기 버튼 생성할 수 있다.
             굳이 SizeBox 를 사용하여 버튼을 감쌀 필요 없지만
