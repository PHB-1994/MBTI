...
class _MapScreenState extends State<MapScreen> {
  ...
  void initState() {
    super.initState();
    _determinePosition(); // 우리가 만들 기능 화면을 시작 시 위치 가져오기 자동 로딩
  }

  Future<void> _determinePosition() async {
    bool serviceEnabled; // 기기의 GPS 가 켜져있는지 확인 기능을 담기 위해 준비한 변수 공간 true false
    ...
    permission =
        await Geolocator.checkPermission(); // 현재 우리 어플이 위치 권한을 가지고 있는지 상태 확인
    // 만약 권한이 거부상태라면   denied = 거부
    if (permission == LocationPermission.denied) {
      // requestPermission = 요청 권한  사용자에게 위치 권한을 허용하시겠습니까? 시스템 팝업 띄우기 기능
      permission = await Geolocator.requestPermission();
      // 팝업창에서 사용자가 거부를 눌렸다면, 좌표 정보를 읽어올 수 없으므로 함수 종료
      if (permission == LocationPermission.denied) return;
    }

    // GPS 접근 가능한 권한이 존재한다면 아래 코드 실행
    // 실제 위도와 경도 데이터를 가져옴 가져온 위치를 position 이라는 변수공간 내부에 저장
    // position = {"latitude" : 39.31231234, "longitude" : 34.4312313, ..... } 와 같은 데이터들이 내부에 저장
  ...
  Widget build(BuildContext context) {
    // Scaffold = .html 과 같이 한 페이지 한 위젯을 나타냄
    return Scaffold(
